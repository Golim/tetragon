...
func TestProcessCacheRemoveStale(t *testing.T) {
	...
	assert.Equal(t, cache.len(), 5)
	assert.NoError(t, testutil.CollectAndCompare(processexecmetrics.ProcessCacheRemovedStale, strings.NewReader(`# HELP tetragon_process_cache_removed_stale_total Number of process cache stale entries removed.
